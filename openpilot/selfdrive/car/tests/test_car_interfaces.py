...
class TestCarInterfaces(OpenpilotTestCase):
  ...
  def test_car_interfaces(self, car_name, fuzzy):
    ...
    car_params = CarInterface.get_params(car_name, fingerprints, car_fw, alpha_long=alpha_long, is_release=False, docs=False)
    car_params_sp = CarInterface.get_params_sp(car_params, car_name, fingerprints, car_fw, alpha_long=alpha_long, is_release_sp=False, docs=False)
    car_params_ic = CarInterface.get_params_ic(car_params, car_name, fingerprints, car_fw, alpha_long=alpha_long, is_release_sp=False, docs=False)
    car_interface = CarInterface(car_params, car_params_sp, car_params_ic)
    ...
    cc_ic_msg = capnp_random_dict(fuzzy, custom.CarControlIC.schema, real_floats=True)
    # Run car interface
    now_nanos = 0
