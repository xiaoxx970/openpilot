...
class SpeedLimitAssist:
  ...
  def update_active_event(self, events_sp: EventsSP) -> None:
    # Nothing is being adjusted when the set speed already matches the target, so stay quiet.
    # A source that drops out and comes back re-reports the same limit and would otherwise
    # announce it again on every recovery.
